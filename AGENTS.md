...
このドキュメントは Cursor の Project Rules として自動認識されます。以下のタイミングで必ず参照してください：

- **作業開始時**: 新しいタスクや作業を依頼された際は、このドキュメントの内容を確認し、返答の文頭に「★」を入れてください
- **実装前**: コードを実装する前に、基本原則と関連ガイドラインを確認してください
- **判断に迷った時**: 実装方法や設計判断に迷った時は、このドキュメントを参照してください
- **ユーザーが明示的に指定した時**: ユーザーが `@AGENTS.md` を指定した場合は、必ず内容を確認してください

このドキュメントは、すべての作業において最優先で適用されるルールです。
...
実装をおこなうときは必ず AGENTS.md を参照した後に実装すること。AGENTS.md からリンクされているドキュメント（[Frontend Guidelines](./docs/frontend-guidelines.md) など）も必ず確認し、内容を理解してから実装を開始すること。

判断に迷った時や、実装方法に自信がない時は、その都度このドキュメントを参照してください。Project Rules として自動認識されていますが、必要に応じて明示的に参照することも重要です。
...
> **重要**: このファイル（AGENTS.md）を確認する際は、上記のリンク先ドキュメント（Frontend Guidelines、Backend Guidelines など）も必ず一度読んで理解すること。リンク先のドキュメントは、AGENTS.mdの基本原則を補完する具体的な実装規約であり、作業開始前に必ず確認する必要がある。
...
### 1. 作業開始前
1. **必ずこのファイル（AGENTS.md）を確認する** - [基本原則](#基本原則)を含むプロジェクトのルール、ガイドライン、コンポーネント設計パターンを理解する。確認したら返答の文頭に「★」を入れる
2. **リンク先のドキュメントも必ず確認する** - AGENTS.md からリンクされているドキュメント（[Frontend Guidelines](./docs/frontend-guidelines.md) など）も必ず一度読んで理解すること。リンク先のドキュメントは、AGENTS.mdの基本原則を補完する具体的な実装規約である
3. **リポジトリ理解の完全性を確認する** - リポジトリの全体構造と方針を 100% 理解できていることを確認する。理解が不十分な場合は、HITL を行い開発者に質問する
4. Issue またはチケットで要件・背景・完了条件を確認
5. 関連ドキュメント（各ガイドライン、コンポーネント仕様）を確認
6. **実装計画を提示し承認を得る** - 十分な理解が蓄積された時点で実装計画を開発者に提示し、承認を受けたのちに作業を開始する
7. 不明点があれば質問・相談する

> **注意**: 判断に迷った時や、実装方法に自信がない時は、その都度このドキュメントを参照してください。「一度だけ確認」ではなく、必要に応じて参照することが重要です。
...
### チェックリスト
- [ ] **AGENTS.md を確認した**（必須）
- [ ] **リンク先のドキュメント（Frontend Guidelines など）も確認した**（必須）
- [ ] **基本原則を理解した**（必須）
