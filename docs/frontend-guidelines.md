...
- CSS 変数を Tailwind クラスに埋め込む際は、`suggestCanonicalClasses` 設定を有効化しているため、`bg-[--token-name]` のような canonical class 形式を使用する（`bg-[var(--token-name)]` のような従来記法は使用しない）
...
- `@radix-ui/react-slot` の `Slot` コンポーネントと `asChild` プロップを使用することで、呼び出し側がタグを差し替えられるようにする。`ref` が必要な場合も `asChild` を使用して対応する（例: `<Text asChild><span ref={ref}>...</span></Text>`）
- スタイルの決定には `cva` (class-variance-authority) を使用し、variant に基づいて適切なクラスを適用する

### シンプルコーディング原則
- **コードは可能な限りシンプルに**: 不要な複雑さを避け、読みやすさと保守性を優先する
- **動的タグ選択を避ける**: variant に基づいて動的にタグを選択するのではなく、デフォルトタグを固定し、`asChild` プロップで必要な場合にタグを差し替えられるようにする（例: `Button`, `Badge`, `Heading` のパターン）
- **forwardRef は必要最小限に**: shadcn/ui の標準パターンに合わせ、基本的に `React.forwardRef` は使用しない。`ref` が必要な場合は `asChild` プロップを使用して対応する（例: `<Text asChild><span ref={ref}>...</span></Text>`）
- **型アサーションを避ける**: TypeScript の型推論を活用し、不要な型アサーション（`as`）は使用しない
- **既存パターンに統一**: 新しいコンポーネントを作成する際は、既存のコンポーネント（`Button`, `Badge` など）と同じパターンに統一する
- **不要な型定義を削減**: 可能な限り TypeScript の型推論に任せ、必要最小限の型定義のみを記述する
- **マッピングオブジェクトを避ける**: 動的タグ選択のためのマッピングオブジェクト（`headingTagMap` など）は使用せず、シンプルな条件分岐で実装する
