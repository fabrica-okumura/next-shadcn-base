...
function postProcessCss() {
  ...
  const filtered = original
    .split('\n') // 行単位に分割
    .filter((line) => !(/[\u3040-\u30ff\u4e00-\u9faf]/.test(line))) // 日本語を含む行を除去
    .join('\n')

  const shifted = shiftRemValues(filtered) // rem 値を 1/10 に変換
...
try {
  ...
  postProcessCss() // 日本語行削除 & rem 調整
  console.log('Tokens built, Japanese lines removed, rem values shifted.') // 正常終了メッセージ
} catch (error) {
